...
MPEG::File::File(FileName file, ID3v2::FrameFactory *frameFactory,
    ...
    : TagLib::File(file)
{
    d = new MPEG::FilePrivate(frameFactory);
...
{
    if (d->hasID3v2)
        d->tag.access<ID3v2::Tag>(ID3v2Index, false)->removeUnsupportedProperties(
            properties);
    else if (d->hasAPE)
        d->tag.access<APE::Tag>(APEIndex, false)->removeUnsupportedProperties(
            properties);
    else if (d->hasID3v1)
        d->tag.access<ID3v1::Tag>(ID3v1Index, false)->removeUnsupportedProperties(
            properties);
}
...
PropertyMap MPEG::File::setProperties(const PropertyMap &properties)
{
    ...
    return d->tag.access<ID3v2::Tag>(ID3v2Index, true)->setProperties(
        properties);
}
    ...
    bool duplicateTags)
{
    ...
    if (ID3v2 & tags)
    {
        ...
        if (ID3v2Tag() && !ID3v2Tag()->isEmpty())
        {
            ...
            insert(ID3v2Tag()->render(id3v2Version), d->ID3v2Location,
                   d->ID3v2OriginalSize);
        ...
        else
        {
            ...
            else
            {
                ...
                d->APEFooterLocation = d->APELocation
                    + apeTag->footer()->completeTagSize() - APE::Footer::size();
...
long MPEG::File::findID3v2()
{
    ...
    if (isValid() && ID3v2::Header::fileIdentifier().size() <= bufferSize())
    {
        ...
        for (buffer = readBlock(bufferSize()); buffer.size() > 0; buffer =
            readBlock(bufferSize()))
        {
            ...
            if (previousPartialMatch >= 0
                && int(bufferSize()) > previousPartialMatch)
            {
                const int patternOffset = (bufferSize() - previousPartialMatch);
                if (buffer.containsAt(ID3v2::Header::fileIdentifier(), 0,
                                      patternOffset))
                {
            ...
            while (firstSynchByte >= 0)
            {
                ...
                firstSynchByte = buffer.find(char(uchar(255)),
                                             firstSynchByte + 1);
            }

            // (3) partial match

            previousPartialMatch = buffer.endsWithPartialMatch(
                ID3v2::Header::fileIdentifier());
