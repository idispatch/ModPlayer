...
namespace TagLib
{
    namespace MPEG
    {
        ...
        {
        public:
            PropertiesPrivate(File *f, Properties::ReadStyle s)
                : file(f),
                  xingHeader(0),
                  style(s),
                  length(0),
...
{
    d = new MPEG::PropertiesPrivate(file, style);
...
void MPEG::Properties::read()
{
    ...
    {
        debug(
            "MPEG::Properties::read() -- Could not find a valid first MPEG frame in the stream.");
        return;
    }
    ...
    int xingHeaderOffset = MPEG::XingHeader::xingHeaderOffset(
        firstHeader.version(), firstHeader.channelMode());
    ...
    if (d->xingHeader->isValid() && firstHeader.sampleRate() > 0
        && d->xingHeader->totalFrames() > 0)
    {
        double timePerFrame = double(firstHeader.samplesPerFrame())
        ...
        d->bitrate =
            d->length > 0 ?
                (int) (d->xingHeader->totalSize() * 8 / length / 1000) : 0;
    }
    else
    {
        ...
        if (firstHeader.frameLength() > 0 && firstHeader.bitrate() > 0)
        {
            ...
            d->length = int(
                float(firstHeader.frameLength() * frames)
