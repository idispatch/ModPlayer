...
const ID3v2::Latin1StringHandler *ID3v2::TagPrivate::stringHandler =
...
String ID3v2::Tag::comment() const
{
    ...
    for (FrameList::ConstIterator it = comments.begin(); it != comments.end();
        ++it)
    {
...
String ID3v2::Tag::genre() const
{
    ...
    TextIdentificationFrame *f =
        static_cast<TextIdentificationFrame *>(d->frameListMap["TCON"].front());
...
PropertyMap ID3v2::Tag::properties() const
{
    ...
    for (FrameList::ConstIterator it = frameList().begin();
        it != frameList().end(); ++it)
    {
...
{
    for (StringList::ConstIterator it = properties.begin();
        it != properties.end(); ++it)
    {
        if (it->startsWith("UNKNOWN/"))
        {
            ...
            for (FrameList::ConstIterator fit = l.begin(); fit != l.end();
                fit++)
        ...
        else
        {
            ...
            if (id == "TXXX")
                ...
                frame = UnsynchronizedLyricsFrame::findByDescription(
                    this, description);
            else if (id == "UFID")
                frame = UniqueFileIdentifierFrame::findByOwner(this,
                                                               description);
            if (frame)
                removeFrame(frame);
        }
...
PropertyMap ID3v2::Tag::setProperties(const PropertyMap &origProps)
{
    ...
    Frame::splitProperties(origProps, properties, tiplProperties,
                           tmclProperties);
    for (FrameListMap::ConstIterator it = frameListMap().begin();
        it != frameListMap().end(); ++it)
    {
        for (FrameList::ConstIterator lit = it->second.begin();
    ...
    for (FrameList::ConstIterator it = framesToDelete.begin();
        it != framesToDelete.end(); ++it)
        removeFrame(*it);
    ...
    for (PropertyMap::ConstIterator it = properties.begin();
        it != properties.end(); ++it)
...
{
    const char *unsupportedFrames[] = { "ASPI", "EQU2", "RVA2", "SEEK", "SIGN",
        "TDRL", "TDTG", "TMOO", "TPRO", "TSOA", "TSOT", "TSST", "TSOP", 0 };
    ...
    for (FrameList::Iterator it = d->frameList.begin();
        it != d->frameList.end(); it++)
    {
        ...
        for (int i = 0; unsupportedFrames[i]; i++)
        {
            ...
            {
                debug(
                    "A frame that is not supported in ID3v2.3 \'"
    ...
    if (frameTDRC)
    {
        ...
        if (content.size() >= 4)
        {
            ...
            if (content.size() >= 10 && content[4] == '-' && content[7] == '-')
            {
                ...
                if (content.size() >= 16 && content[10] == 'T'
                    && content[13] == ':')
                {
                    ID3v2::TextIdentificationFrame *frameTIME =
                        new ID3v2::TextIdentificationFrame("TIME",
                                                           String::Latin1);
                    frameTIME->setText(
                        content.substr(11, 2) + content.substr(14, 2));
...
ByteVector ID3v2::Tag::render(int version) const
{
    ...
    for (FrameList::Iterator it = frameList.begin(); it != frameList.end();
        it++)
    {
        ...
        {
            debug(
                "A frame of unsupported or unknown type \'"
                    + String((*it)->header()->frameID())
                    + "\' has been discarded");
            continue;
        }
...
void ID3v2::Tag::parse(const ByteVector &origData)
{
    ...
    while (frameDataPosition
        < frameDataLength - Frame::headerSize(d->header.majorVersion()))
    {
        ...
        if (data.at(frameDataPosition) == 0)
        {
            ...
            {
                debug(
                    "Padding *and* a footer found.  This is not allowed by the spec.");
            }
