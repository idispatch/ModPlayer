...
{
    return TagLib::ID3v2::HeaderPrivate::size;
}
...
Header::Header()
{
    d = new TagLib::ID3v2::HeaderPrivate;
}
...
void Header::parse(const ByteVector &data)
{
    ...
    if (sizeData.size() != 4)
    {
        d->tagSize = 0;
        debug(
            "TagLib::ID3v2::Header::parse() - The tag size as read was 0 bytes!");
        return;
    }
    ...
    for (ByteVector::Iterator it = sizeData.begin(); it != sizeData.end(); it++)
    {
        if (uchar(*it) >= 128)
        {
            d->tagSize = 0;
            debug(
                "TagLib::ID3v2::Header::parse() - One of the size bytes in the id3v2 header was greater than the allowed 128.");
            return;
        }
    }
