...
namespace TagLib
{
    namespace FLAC
    {
        ...
        {
        public:
            PropertiesPrivate(ByteVector d, long st, Properties::ReadStyle s) :
                    data(d), streamLength(st), style(s), length(0), bitrate(0), sampleRate(
                            0), sampleWidth(0), channels(0), sampleFrames(0)
            {
            }

            ByteVector data;
            long streamLength;
            Properties::ReadStyle style;
            int length;
            int bitrate;
            int sampleRate;
            int sampleWidth;
...
FLAC::Properties::Properties(ByteVector data, long streamLength,
        ReadStyle style) :
        AudioProperties(style)
{
    d = new FLAC::PropertiesPrivate(data, streamLength, style);
    read();
}
...
FLAC::Properties::Properties(File *file, ReadStyle style) :
        AudioProperties(style)
{
    d = new FLAC::PropertiesPrivate(file->streamInfoData(),
            file->streamLength(), style);
    read();
}
...
void FLAC::Properties::read()
{
    ...
    {
        debug(
                "FLAC::Properties::read() - FLAC properties must contain at least 18 bytes.");
        return;
    }
    ...
    d->bitrate =
            d->length > 0 ? ((d->streamLength * 8UL) / d->length) / 1000 : 0;
