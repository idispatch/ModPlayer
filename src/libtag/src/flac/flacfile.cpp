...
namespace TagLib
{
    namespace FLAC
    {
        ...
        public:
            FilePrivate() :
                    ID3v2FrameFactory(ID3v2::FrameFactory::instance()), ID3v2Location(
                            -1), ID3v2OriginalSize(0), ID3v1Location(-1), properties(
                            0), flacStart(0), streamStart(0), streamLength(0), scanned(
                            false), hasXiphComment(false), hasID3v2(false), hasID3v1(
                            false)
            {
            }
...
{
    if (d->hasXiphComment)
        d->tag.access<Ogg::XiphComment>(FlacXiphIndex, false)->removeUnsupportedProperties(
                unsupported);
    if (d->hasID3v2)
        d->tag.access<ID3v2::Tag>(FlacID3v2Index, false)->removeUnsupportedProperties(
                unsupported);
    if (d->hasID3v1)
        d->tag.access<ID3v1::Tag>(FlacID3v1Index, false)->removeUnsupportedProperties(
                unsupported);
}

PropertyMap FLAC::File::setProperties(const PropertyMap &properties)
{
    return d->tag.access<Ogg::XiphComment>(FlacXiphIndex, true)->setProperties(
            properties);
}
...
// Replace metadata blocks
    ...
    {
        newBlocks.append(
                new UnknownMetadataBlock(MetadataBlock::VorbisComment,
                        d->xiphCommentData));
        foundVorbisCommentBlock = true;
    }
    d->blocks = newBlocks;
    ...
    if (ID3v2Tag())
    {
        if (d->hasID3v2)
        {
            if (d->ID3v2Location < d->flacStart)
                debug(
                        "FLAC::File::save() -- This can't be right -- an ID3v2 tag after the "
                                "start of the FLAC bytestream?  Not writing the ID3v2 tag.");
            else
                insert(ID3v2Tag()->render(), d->ID3v2Location,
                        d->ID3v2OriginalSize);
        }
        else
...
// Look for an ID3v2 tag
    ...
    if (d->ID3v2Location >= 0)
    {
        ...
        d->tag.set(FlacID3v2Index,
                new ID3v2::Tag(this, d->ID3v2Location, d->ID3v2FrameFactory));
...
// Search through the remaining metadata
    while (!isLastBlock)
    {
        ...
        if (blockType == MetadataBlock::VorbisComment)
        {
            ...
            {
                debug(
                        "FLAC::File::scan() -- multiple Vorbis Comment blocks found, using the first one");
            }
        }
        else if (blockType == MetadataBlock::Picture)
        {
            ...
            {
                debug(
                        "FLAC::File::scan() -- invalid picture found, discarting");
                delete picture;
            }
        }
