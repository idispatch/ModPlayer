...
ByteVector Ogg::File::packet(uint i)
{
    ...
    while (d->packetToPageMap.size() <= i)
    {
        ...
        {
            debug(
                "Ogg::File::packet() -- Could not find the requested packet.");
...
void Ogg::File::setPacket(uint i, const ByteVector &p)
{
    while (d->packetToPageMap.size() <= i)
    {
        ...
        {
            debug(
                "Ogg::File::setPacket() -- Could not set the requested packet.");
            return;
        }
    }
...
bool Ogg::File::save()
{
    ...
    for (List<int>::ConstIterator it = d->dirtyPages.begin();
        it != d->dirtyPages.end(); ++it)
    {
...
bool Ogg::File::nextPage()
{
    ...
    else
    {
        ...
        if (d->currentPage->header()->lastPacketCompleted())
            currentPacket = d->currentPage->firstPacketIndex()
                + d->currentPage->packetCount();
        else
            currentPacket = d->currentPage->firstPacketIndex()
                + d->currentPage->packetCount() - 1;
...
void Ogg::File::writePageGroup(const List<int> &thePageGroup)
{
    ...
    if (!d->dirtyPages.contains(
        d->pages[pageGroup.front()]->firstPacketIndex()))
        packets.append(d->pages[pageGroup.front()]->packets().front());

    int previousPacket = -1;
    int originalSize = 0;

    for (List<int>::ConstIterator it = pageGroup.begin(); it != pageGroup.end();
        ++it)
    {
    ...
    if (pages.back()->header()->pageSequenceNumber() != pageGroup.back())
    {
        ...
        int numberOfNewPages = pages.back()->header()->pageSequenceNumber()
            - pageGroup.back();
        ...
        for (int i = 0; i < pageGroup.back(); i++)
        {
            ...
            {
                debug(
                    "Ogg::File::writePageGroup() -- Page sequence is broken in original file.");
                break;
            }
        }
        ...
        {
            Ogg::Page *newPage = (*pageIter)->getCopyWithNewPageSequenceNumber(
    ...
    ByteVector data;
    for (List<Page *>::ConstIterator it = pages.begin(); it != pages.end();
        ++it)
    ...
    for (List<Page *>::ConstIterator it = renumberedPages.begin();
        it != renumberedPages.end(); ++it)
    {
        const unsigned int index = (*it)->header()->pageSequenceNumber();
        if (index < d->pages.size())
        {
