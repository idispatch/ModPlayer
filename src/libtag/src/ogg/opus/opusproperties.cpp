...
namespace TagLib
{
    namespace Ogg
    {
        namespace Opus
        {
            ...
            {
            public:
                PropertiesPrivate(File *f, AudioProperties::ReadStyle s)
                    : file(f),
                      style(s),
                      length(0),
                      inputSampleRate(0),
...
Opus::Properties::Properties(File *file, AudioProperties::ReadStyle style)
    : AudioProperties(style)
{
    d = new Opus::PropertiesPrivate(file, style);
    read();
}
...
void Opus::Properties::read()
{
    ...
    if (first && last)
    {
        ...
        debug(
            "Opus::Properties::read() -- Could not find valid first and last Ogg pages.");
}
