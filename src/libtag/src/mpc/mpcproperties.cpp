...
namespace TagLib
{
    namespace MPC
    {
        ...
        {
        public:
            PropertiesPrivate(long length, Properties::ReadStyle s)
                : streamLength(length),
                  style(s),
                  version(0),
                  length(0),
...
MPC::Properties::Properties(const ByteVector &data, long streamLength,
    ReadStyle style)
    : AudioProperties(style)
{
    d = new MPC::PropertiesPrivate(streamLength, style);
    readSV7(data);
}
...
void MPC::Properties::readSV8(File *file)
{
    ...
    while (!readSH && !readRG)
    {
        ...
        if (packetType == "SH")
        {
            ...
            std::bitset<16> flags(
                TAGLIB_CONSTRUCT_BITSET(data.toUShort(pos, true)));
            pos += 2;

            d->sampleRate = sftable[flags[15] * 4 + flags[14] * 2 + flags[13]];
            d->channels = flags[7] * 8 + flags[6] * 4 + flags[5] * 2 + flags[4]
                + 1;

            if ((d->sampleFrames - begSilence) != 0)
                d->bitrate = (int) (d->streamLength * 8.0 * d->sampleRate
                    / (d->sampleFrames - begSilence));
...
void MPC::Properties::readSV7(const ByteVector &data)
{
    ...
    d->length =
        d->sampleRate > 0 ?
            (d->sampleFrames + (d->sampleRate / 2)) / d->sampleRate : 0;

    if (!d->bitrate)
        d->bitrate =
            d->length > 0 ? ((d->streamLength * 8L) / d->length) / 1000 : 0;
}
