...
namespace TagLib
{
    namespace RIFF
    {
        ...
        public:
            FilePrivate()
                : endianness(RIFF::File::BigEndian), size(0)
            {
            }
            RIFF::File::Endianness endianness;
            ByteVector type;
            TagLib::uint size;
            ByteVector format;
...
void RIFF::File::setChunkData(uint i, const ByteVector &data)
{
    ...
    d->size += ((data.size() + 1) & ~1)
        - (d->chunks[i].size + d->chunks[i].padding);
    insert(ByteVector::fromUInt(d->size, d->endianness == BigEndian), 4, 4);

    // Now update the specific chunk

    writeChunk(chunkName(i), data, d->chunks[i].offset - 8,
               d->chunks[i].size + d->chunks[i].padding + 8);
    ...
    for (i++; i < d->chunks.size(); i++)
        d->chunks[i].offset = d->chunks[i - 1].offset + 8
    ...
    bool alwaysCreate)
{
    ...
    {
        debug(
            "RIFF::File::setChunkData - alwaysCreate should be used for only \"LIST\" chunks.");
        return;
    }
    ...
    writeChunk(name, data, offset, std::max<long>(0, length() - offset),
               (offset & 1) ? 1 : 0);
...
void RIFF::File::read()
{
    ...
    while (tell() + 8 <= length())
    {
        ...
        {
            debug(
                "RIFF::File::read() -- Chunk '" + chunkName
                    + "' has invalid ID");
            setValid(false);
            break;
        }

        if (static_cast<ulonglong>(tell()) + chunkSize
    ...
    ulong offset, ulong replace, uint leadingPadding)
{
    ...
    combined.append(
        ByteVector::fromUInt(data.size(), d->endianness == BigEndian));
