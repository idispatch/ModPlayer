...
{
    d = new RIFF::WAV::PropertiesPrivate();
    read(data);
}

RIFF::WAV::Properties::Properties(const ByteVector &data, uint streamLength,
    ReadStyle style)
    : AudioProperties(style)
{
    d = new RIFF::WAV::PropertiesPrivate(streamLength);
    read(data);
}
...
void RIFF::WAV::Properties::read(const ByteVector &data)
{
    ...
    if (d->channels > 0 && d->sampleWidth > 0)
        d->sampleFrames = d->streamLength
            / (d->channels * ((d->sampleWidth + 7) / 8));
}
