...
const RIFF::Info::StringHandler *RIFF::Info::TagPrivate::stringHandler =
    &defaultStringHandler;

RIFF::Info::Tag::Tag(const ByteVector &data)
    : TagLib::Tag(), d(new RIFF::Info::TagPrivate())
{
    parse(data);
}
...
void RIFF::Info::Tag::parse(const ByteVector &data)
{
    ...
    while (p < data.size())
    {
        ...
        d->fieldListMap[data.mid(p, 4)] = TagPrivate::stringHandler->parse(
            data.mid(p + 8, size));
