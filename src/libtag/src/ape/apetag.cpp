...
static const char *keyConversions[][2] = { { "TRACKNUMBER", "TRACK" }, { "DATE",
    "YEAR" }, { "ALBUMARTIST", "ALBUM ARTIST" }, { "DISCNUMBER", "DISC" }, {
    "REMIXER", "MIXARTIST" } };
...
PropertyMap APE::Tag::setProperties(const PropertyMap &origProps)
{
    ...
    for (uint i = 0; i < keyConversionsSize; ++i)
        ...
        {
            properties.insert(keyConversions[i][1],
                              properties[keyConversions[i][0]]);
    ...
    for (; remIt != itemListMap().end(); ++remIt)
    {
        ...
        if (!key.isNull() && remIt->second.type() == APE::Item::Text
            && !properties.contains(key))
            toRemove.append(remIt->first);
    }

    for (StringList::Iterator removeIt = toRemove.begin();
        removeIt != toRemove.end(); removeIt++)
        removeItem(*removeIt);
    ...
    for (; it != properties.end(); ++it)
    {
        ...
        else if (!(itemListMap().contains(tagName))
            || !(itemListMap()[tagName].values() == it->second))
        {
            if (it->second.size() == 0)
                removeItem(tagName);
            else
...
bool APE::Tag::checkKey(const String &key)
{
    ...
    if (upperKey == "ID3" || upperKey == "TAG" || upperKey == "OGGS"
        || upperKey == "MP+")
        return false;
    return true;
}
