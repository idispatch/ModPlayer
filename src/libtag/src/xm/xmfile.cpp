...
class TAGLIB_EXPORT SkipReader : public Reader
{
public:
    SkipReader(uint size)
        : m_size(size)
    {
    }
...
template<typename T>
class TAGLIB_EXPORT ValueReader : public Reader
{
public:
    ValueReader(T &value)
        : value(value)
...
class TAGLIB_EXPORT StringReader : public ValueReader<String>
{
public:
...
{
public:
    ByteReader(uchar &byte)
        : ValueReader<uchar>(byte)
    {
    }
...
XM::File::File(FileName file, bool readProperties,
    ...
    : Mod::FileBase(file)
{
    d = new XM::FilePrivate(propertiesStyle);
    if (isOpen())
        read(readProperties);
}
...
AudioProperties *XM::File::audioProperties() const
{
    return &d->properties;
}
...
void XM::File::read(bool)
{
    ...
    StructReader header;
    header.u16L(length).u16L(restartPosition).u16L(channels).u16L(patternCount).u16L(
        instrumentCount).u16L(flags).u16L(tempo).u16L(bpmSpeed);
    ...
    for (ushort i = 0; i < patternCount; ++i)
    {
        ...
        uint count = pattern.read(*this, patternHeaderLength - 4U);
        READ_ASSERT(
            count == std::min(patternHeaderLength - 4U, (ulong )pattern.size()));
    ...
    for (ushort i = 0; i < instrumentCount; ++i)
    {
        ...
        instrument.string(instrumentName, 22).byte(instrumentType).u16L(
            sampleCount);
        ...
        READ_ASSERT(
            count
                == std::min(instrumentHeaderSize,
                            (ulong )instrument.size() + 4));
        ...
        if (sampleCount > 0)
        {
            ...
            READ_ASSERT(
                instrumentHeaderSize >= count + 4 && readU32L(sampleHeaderSize));
            // skip unhandeled header proportion:
            ...
            for (ushort j = 0; j < sampleCount; ++j)
            {
                ...
                String sampleName;
                StructReader sample;
                sample.u32L(sampleLength).u32L(loopStart).u32L(loopLength).byte(
                    volume).byte(finetune).byte(sampleType).byte(panning).byte(
                    noteNumber).byte(compression).string(sampleName, 22);

                uint count = sample.read(*this, sampleHeaderSize);
                READ_ASSERT(
                    count == std::min(sampleHeaderSize, (ulong )sample.size()));
                // skip unhandeled header proportion:
