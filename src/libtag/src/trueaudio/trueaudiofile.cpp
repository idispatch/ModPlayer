...
TrueAudio::File::File(FileName file, ID3v2::FrameFactory *frameFactory,
    ...
    : TagLib::File(file)
{
    d = new TrueAudio::FilePrivate(frameFactory);
    if (isOpen())
        read(readProperties, propertiesStyle);
}
...
PropertyMap TrueAudio::File::properties() const
{
    ...
    // within TagUnion.
    if (d->hasID3v2)
        return d->tag.access < ID3v2::Tag
            > (TrueAudioID3v2Index, false)->properties();
    if (d->hasID3v1)
        return d->tag.access < ID3v1::Tag
            > (TrueAudioID3v1Index, false)->properties();
    return PropertyMap();
}
...
{
    if (d->hasID3v2)
        d->tag.access < ID3v2::Tag
            > (TrueAudioID3v2Index, false)->removeUnsupportedProperties(
                unsupported);
}
...
{
    if (d->hasID3v1)
        d->tag.access < ID3v1::Tag
            > (TrueAudioID3v1Index, false)->setProperties(properties);
    return d->tag.access < ID3v2::Tag
        > (TrueAudioID3v2Index, true)->setProperties(properties);
}
