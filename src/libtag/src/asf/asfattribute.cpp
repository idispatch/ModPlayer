...
namespace TagLib
{
    namespace ASF
    {
        class AttributePrivate : public RefCounter
        {
        public:
            AttributePrivate()
                    : type(ASF::Attribute::UnicodeType),
...
ByteVector ASF::Attribute::render(const String &name, int kind) const
{
    ...
    if (kind == 0)
    {
        data = File::renderString(name, true)
                ...
                + ByteVector::fromShort(data.size(), false) + data;
    }
    else
    {
        ...
        data = ByteVector::fromShort(kind == 2 ? d->language : 0, false)
                ...
                + ByteVector::fromShort((int) d->type, false)
                + ByteVector::fromUInt(data.size(), false) + nameData + data;
    }
