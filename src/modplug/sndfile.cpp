...
CSoundFile::CSoundFile()
{
    ...
    m_nBufferCount = 0;
    m_nCurrentPattern = 0;
    m_nDefaultGlobalVolume = 0;
    m_nDefaultSpeed = 0;
    m_nDefaultTempo = 0;
    m_nFrameDelay = 0;
    m_nGlobalFadeMaxSamples = 0;
    m_nGlobalVolume = 0;
    m_nInitialRepeatCount = 0;
...
void CSoundFile::MuteChannel(UINT channel, bool mute)
{
    if(channel >= MAX_CHANNELS) {
        return;
    }
    if(mute) {
        Chn[channel].dwFlags |= CHN_MUTE;
    } else {
        Chn[channel].dwFlags &= ~CHN_MUTE;
    }
}

BOOL CSoundFile::IsChannelMuted(UINT channel) const
{
    if(channel >= MAX_CHANNELS) {
        return FALSE;
    }
    return (Chn[channel].dwFlags & CHN_MUTE) ? TRUE: FALSE;
}
